...
public class SurivivalStats : MonoBehaviour
{
    [Header("Hunger Setting")]
    public float maxHunger = 100;       //최대 허기량
    public float currentHunger;         //현재 허기향
    ...
    public void RepaurSuit(float amount)        //우주복 수리 (크리스탈로 제작한 수리 키트 사용)
    {
        if (isGameOver || isPaused) return;

        currentHunger = Mathf.Min(maxSuitDurability, currentSuitDurability + amount);       ///100 수치 이상 가지 않게 설정
    ...
    public float GetSuitDuravilityPercentage()      //슈트 % 리턴 함수
    {
